...
# Titanic-specific preprocessing functions
def preprocess_titanic_data(file_path):
    data = pd.read_csv(file_path)
    label_col = identify_label(data)
    categorical_cols, numerical_cols = identify_columns(data, label_col)
    data = one_hot_encode(data, categorical_cols)
    X = data.drop(columns=[label_col]).values
    y = data[label_col].values.astype(int)
    features = data.drop(columns=[label_col]).columns.tolist()
    return X, y, features

def identify_label(data):
...
def identify_columns(data, label_col):
    categorical_cols = []
    numerical_cols = []
...
# Random Forest functions (unchanged)
...
def run_random_forest(filename, nTrees=[1, 5, 10, 20, 30, 40, 50], k=5):
    ...
    if "titanic" in filename.lower():
        X, y, features = preprocess_titanic_data(filename)
...
if __name__ == "__main__":
    run_random_forest("Random Forest/titanic.csv")